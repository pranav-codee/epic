    ...
    <>
      ...
      <div className="dashboard-grid">
        ...
        <div className="stat">
          ...
          <div className="l">All my tickets</div>
        </div>
      </div>
      <h3 style={{ marginTop: 32 }}>Recent</h3>
      {tickets.slice(0, 5).map((t) => (
        <div key={t.id} className="card">
          <Link to={`/employee/tickets/${t.id}`}>
          ...
          <div className="muted" style={{ marginTop: 4 }}>
            {formatUtcDateTime(t.created_at)}
          </div>
        </div>
      ))}
      {tickets.length === 0 && (
        <p className="muted">
          You have no tickets yet. <Link to="/employee/new">Create one</Link>.
        </p>
      )}
    </>
  );
